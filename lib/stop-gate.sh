...
CLINE_PLUGIN_ROOT="${CLINE_PLUGIN_ROOT:-$(cd "$SCRIPT_DIR/../.." && pwd)}"
CACHE_DIR="${PLUGIN_ROOT_OVERRIDE:-$CLINE_PLUGIN_ROOT}/cache"
...
if [ "$STOP_HOOK_ACTIVE" = "true" ]; then
    # Claude Code Stop schema rejects hookSpecificOutput with custom fields; {} is the canonical schema-valid pass-through.
    printf '{}\n'
    exit 0
fi
...
if [ "$TURN_STATUS" = "in_progress" ]; then
    if [ -f "$CLINE_PLUGIN_ROOT/lib/repl-invoke.sh" ]; then
