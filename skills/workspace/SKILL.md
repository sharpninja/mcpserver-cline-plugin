...
Prefer a trusted existing marker from the active workspace. If the target workspace already has `AGENTS-README-FIRST.yaml`, validate it with the plugin marker resolver before any MCP call. A trusted marker means the workspace is already registered enough to continue normal plugin bootstrap.
...
2. Validate any existing target marker before MCP writes.
...
- Marker trust succeeds and reports the expected workspace name and base URL.
