...
description: Use when Cline discovers an incidental bug while working on another task and should submit it to MCP Server triage without changing focus.
version: 0.1.0
---
...
payload:
  ...
  params:
    title: mcpserver-cline-plugin rejects triage_status
    summary: The plugin status command does not route to workflow.triage status.
    component: mcpserver-cline-plugin
    errorSignature: triage_status_rejected
    reporterAgent: Cline
```
