...
Use the Cline plugin memory surface. Do not invent memories. Prefer the plugin REPL/workflow wrapper over local files. Host-registered `memory_*` MCP tools are aliases for `workflow.memory.*` methods invoked through `lib/repl-invoke.ps1` or `skills/memory/scripts/invoke.ps1`.

## Injection

Cline does not expose Claude-style `UserPromptSubmit`. The always-on request-boundary path is `session_begin_turn` in the TypeScript MCP server (`src/index.ts`). That handler loads `memory-descriptor.json` and injects required memories into the tool result, including the explicit `None` fallback. Render required memories exactly as:
...
`workflow.memory.*` is a plugin workflow/REPL namespace. Native `/mcp-transport` discovery may show `memory_*` names when this plugin's MCP server is configured; those names still dispatch to `workflow.memory.*`.
