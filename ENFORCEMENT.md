# Per-Turn Enforcement Protocol (Cline) - v4 Shared Protocol

This plugin implements the McpServer **v4 Shared Enforcement Protocol** for Cline agent sessions.
...
Cline-visible MCP tools use this plugin's host-facing names, such as
`session_begin_turn`, `todo_query`, and `req_generate_document`. The helper
scripts still call `workflow.sessionlog.*`, `workflow.todo.*`, and
...
plugin unavailable solely because those names are absent. Validate Cline's
configured MCP server tools and the helper scripts separately.
